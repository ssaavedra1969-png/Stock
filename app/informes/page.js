...
import { formatDate, formatDateTime, todayISO, toMillis } from '@/lib/utils';
import {
  IconFilter,
  IconFileSpreadsheet,
  IconPrinter,
...
export default function Informes() {
  ...
  const stockRows = useMemo(() => {
    ...
    for (const r of baseRecords) {
      ...
      it.count += 1;
      if (r.carga === 'Entrada') it.entradas += num;
      else it.salidas += num;
    }
    return [...map.values()]
  ...
  const totals = useMemo(() => {
    if (tipo === 'stock' || tipo === 'comparativo') {
      ...
      return { count: stockRows.length, totalMovs, entradas, salidas, stock };
    }
    if (tipo === 'por-planta') {
      return {
  ...
  const filterLabel = useMemo(() => {
    ...
    return parts.join('  ·  ');
  }, [periodoLabel, producto, planta, contraparte]);
  ...
  async function handleExportPDF() {
    try {
      ...
      autoTable(doc, {
        ...
        margin: { top: margin + 32, right: margin, bottom: 16, left: margin },
      });
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="card space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Producto</span>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-white text-slate-900 shadow-2xl">
        ...
        <div className="grid grid-cols-2 gap-3 border-b border-slate-200 px-6 py-5 sm:grid-cols-4">
          {tipo === 'stock' || tipo === 'comparativo' ? (
            <>
              ...
              <ResumenChip label="Diferencia (tn)" value={fmtNum(totals.stock)} accent />
            </>
          ) : tipo === 'por-planta' ? (
            <>
            ...
            <>
              ...
              <ResumenChip label="Periodo" value={desde || hasta ? `${formatDate(desde || todayISO())} → ${formatDate(hasta || todayISO())}` : 'Completo'} />
            </>
          )}
        </div>
        ...
        <div className="flex flex-col gap-6 px-6 py-6 lg:flex-row lg:items-center">
          <div className="flex flex-col items-center gap-3">
            <DonutChart
              data={pieData}
              centerTop={tipo === 'por-planta' ? 'Stock (tn)' : esMovimiento ? 'Total (tn)' : 'Diferencia (tn)'}
              centerBottom={
                tipo === 'por-planta'
                  ? fmtNum(totals.stock)
                  : esMovimiento
